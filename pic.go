...
// Helper to round an integer to the nearest multiple of 8 (AI friendly)
func PicSnap8(n int) int {
	if n <= 0 {
		return 8
	}
	remainder := n % 8
...
func Pic2DSnap8(w, h int) (int, int) {
	return PicSnap8(w), PicSnap8(h)
}
...
// All inputs and outputs are snapped to the nearest multiple of 8 (AI friendly).
func PicAdjustReso(w, h, target int) (int, int) {
	targetSnapped := PicSnap8(target)
	...
	finalW := PicSnap8(int(math.Round(newWFloat)))
	finalH := PicSnap8(int(math.Round(newHFloat)))
